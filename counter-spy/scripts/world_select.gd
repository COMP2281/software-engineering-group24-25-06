...
# NOTE: the way we load levels is different, so
#	don't need these packed scenes
@export var ai_scene: PackedScene
@export var cyber_security_scene: PackedScene
@export var data_analytics_scene: PackedScene

# NOTE: should be referenced with $ instead of passed in through editor?
...
var is_in_area: bool = false  # Flag to track if the player is inside the area
var menu_open: bool = false  # Track menu state
...
func close_menu():
	...
	self.visible = false
	menu_open = false
	proto_controller.set_all_inputs(true)  # Enable movements
