...
func readProgress(player_name : String) -> void:
...
func _doesTopicMatch(questionMask : int, requestedMask : int) -> bool:
	...
	var lowBitsMatch = bool((questionMask & TOPIC_LOW_BITS) & (requestedMask & TOPIC_LOW_BITS));
	
	return lowBitsMatch;
	
# Difficulty of a question relative to a player's skill
func _relativeDifficulty(question : Question) -> float:
	const playerSkillEffect = 0.2; # Varify question difficulty by at most 0.2 (+-)
	# TODO: check math
	var playerSkillModifier = (1.0 - _playerSkill - 0.5) * playerSkillEffect * 2 + 1.0; # Value ranging [0.8, 1.2]
	
	return clamp(question.difficulty * playerSkillModifier, 0.0, 1.0);
	
func _calculateAnsweringTime(question : Question) -> float:
