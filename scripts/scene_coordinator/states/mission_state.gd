...
# TODO: not messing with inheritance for now, but this can just be put in base class right?
func _ready() -> void:
	SceneCoordinator.change_scene.connect(func(state: String, _metadata): finished.emit(state))

func enter(_previous_state_path: String, _data := {}) -> void:
	var enemy_hunt_scene: Node3D = get_tree().get_nodes_in_group("mission_scene_group")[0]
